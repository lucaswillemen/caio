...
if (isset($user['indicador_id'])) {
	if ($user['indicador_id'] != 0) {
		...
		$conn -> exec("UPDATE `user` SET `ponto`= `ponto`+'".$pacote['nv1']."', `saldo` =  `saldo`+'".$pacote['nv1']."' WHERE `user`.`id` = '".$user['indicador_id']."'");
		$indicador_nv1 = $conn->query("SELECT * FROM `user` WHERE `id` = '".$user['indicador_id']."'")->fetch(PDO::FETCH_ASSOC);
		$conn -> exec("INSERT INTO `extrato`(`valor`, `ponto`, `des`, `user_id`) VALUES ('".$pacote['nv1']."','".$pacote['nv1']."', 'Bônus nível 1', '".$indicador_nv1['id']."')");
...
$conn -> exec("INSERT INTO `caixa`(`valor`, `descricao`) VALUES ('$valor_pacote', 'Ativação de pacote')");
...
$response = array('vencimento' => $novo_vencimento);
